...
tools/run_seed_sweep.py) and draws three panels:
  ...
  scatter — per-seed scatter of PSNR@rho=.20 per ordering (jittered) with the
            mean+/-sigma band overlaid: shows how tight the seed turbulence is.
  hist    — histogram of the PSNR@rho=.20 endpoints per ordering with a fitted
...
By default the three panels are composed into one figure (seed_variance.{pdf,svg}).
With --separate each panel is written as its own standalone figure
(seed_variance_{band,scatter,hist}.{pdf,svg}) with no panel title — the per-figure
caption then lives in the paper / typst block.

Usage:
    python tools/render_seed_variance.py \
        --base results/training/2_direct_training/random_seed/div2k_8q
    python tools/render_seed_variance.py --base <...> --separate
"""
...
def _draw_band(ax, orderings, per_ordering, classical):
    """Panel: PSNR vs rho, mean +/- sigma band + min-max whiskers."""
    for o in orderings:
        ...
        ax.fill_between(xs, mean - std, mean + std, color=color, alpha=0.20, lw=0)
        ax.plot(xs, mean, ls, color=color, marker="o", ms=4, lw=1.8,
                label=f"{lab}  (n={n})")
        ax.vlines(xs, lo, hi, color=color, lw=0.8, alpha=0.6)
    if classical:
        ax.plot(RHO_X, [classical["block_fft_8"]["psnr"][r] for r in RHOS],
                "k--", lw=1.4, label="block-FFT 8×8", zorder=1)
        ax.plot(RHO_X, [classical["block_dct_8"]["psnr"][r] for r in RHOS],
...
def _draw_scatter(ax, orderings, per_ordering, fft20):
    """Panel: per-seed scatter @ rho=.20 + mean+/-sigma error bar."""
    ...
    for i, o in enumerate(orderings):
        ...
        ax.errorbar(i, m, yerr=sd, fmt="_", color="black", ms=18, lw=1.6, capsize=4,
                    zorder=5)
    if fft20 is not None:
        ax.axhline(fft20, color="k", ls="--", lw=1.2, label="block-FFT 8×8")
        ax.legend(frameon=False, fontsize=7, loc="lower right")
    ax.set_xticks(range(len(orderings)))
    ax.set_xticklabels([STYLE[o][2] for o in orderings], fontsize=7.5)
    ax.set_ylabel(r"test PSNR @ $\rho=.20$  (dB)", fontsize=8.5)


def _draw_hist(ax, orderings, per_ordering, fft20):
    """Panel: histogram @ rho=.20 + fitted normal."""
    all_vals = []
    for o in orderings:
        vals = np.array([v["0.2"] for v in per_ordering[o]["per_seed"].values()
                         if "0.2" in v])
    ...
    flat = (np.concatenate([v for v in all_vals if v.size])
            if any(v.size for v in all_vals) else np.array([]))
    if flat.size:
        ...
        for o, vals in zip(orderings, all_vals):
            if vals.size == 0:
                continue
            color, _ls, lab = STYLE[o]
            ax.hist(vals, bins=bins, density=True, color=color, alpha=0.30,
                    label=f"{lab}")
            m, sd = float(vals.mean()), float(vals.std(ddof=1) if vals.size > 1 else 0.0)
            ax.plot(xline, _normal_pdf(xline, m, sd), color=color, lw=1.6)
        if fft20 is not None and lo - pad <= fft20 <= hi + pad:
            ax.axvline(fft20, color="k", ls="--", lw=1.2)
...
def main() -> int:
    ...
    ap.add_argument("--separate", action="store_true", default=False,
                    help="Emit one standalone figure per panel "
                         "(seed_variance_{band,scatter,hist}) with no panel "
                         "title, instead of the composite.")
    args = ap.parse_args()
    ...
    if args.separate:
        specs = [
            ("seed_variance_band", (4.9, 3.7),
             lambda ax: _draw_band(ax, orderings, per_ordering, classical)),
            ("seed_variance_scatter", (4.3, 3.7),
             lambda ax: _draw_scatter(ax, orderings, per_ordering, fft20)),
            ("seed_variance_hist", (4.6, 3.7),
             lambda ax: _draw_hist(ax, orderings, per_ordering, fft20)),
        ]
    ...
    _draw_band(axL, orderings, per_ordering, classical)
    axL.set_title("mean $\\pm$ $\\sigma$ across seeds", fontsize=9)
    _draw_scatter(axM, orderings, per_ordering, fft20)
    axM.set_title("per-seed spread @ $\\rho=.20$", fontsize=9)
    _draw_hist(axR, orderings, per_ordering, fft20)
