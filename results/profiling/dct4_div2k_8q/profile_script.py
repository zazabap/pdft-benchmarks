#!/usr/bin/env python3
"""Profile canonical DCT-IV (pdft.DCT4Basis) training on DIV2K-8q: attribute
per-step wall-clock to forward / top-k / inverse / backward / optimizer.
...
(GPU, warmed, block_until_ready, median of N reps):
  ...
  forward transform   : vmap _apply_circuit(tensors, code)             -> realize
  top-k truncate      : vmap topk_truncate(pred, k)  (full 65536 sort) -> realize
  inverse transform   : vmap _apply_circuit(conj tensors, inv_code)    -> realize
  loss forward (full) : mean batched loss = stacked_loss               (fwd+topk+inv+resid)
  value_and_grad      : jax.value_and_grad(stacked_loss)               (fwd + backward)
  full step           : the real fused step_fn                         (vag + optimizer)
  validation pass     : mean batched loss over the val stack (once/epoch)
...
MEMORY: a batch-50 forward+backward graph peaks ~31 GB. To avoid two such
graphs co-residing (OOM on a 48 GB card), each heavy executable runs in its OWN
subprocess (`--group vag` / `--group step`); the forward-only pieces share one
(`--group light`). The parent (no GPU) orchestrates and aggregates.

Needs pdft>=0.2.2 with DCT4Basis (dct4 worktree first on PYTHONPATH).
Run (parent picks up CUDA_VISIBLE_DEVICES from the env):
  ...
  PYTHONPATH=/workspaces/parametric-dft-paper/pdft-dct4main/src \
    .venv/bin/python results/profiling/dct4_div2k_8q/profile_script.py
"""
...
def run_group(group: str) -> int:
    ...
    if not hasattr(pdft, "DCT4Basis"):
        print(f"FATAL: pdft at {pdft.__file__} lacks DCT4Basis.", file=sys.stderr)
        return 3
    dev = jax.devices()[0]
    if dev.platform not in ("gpu", "cuda"):
        print("FATAL: not on GPU; refusing CPU profile.", file=sys.stderr)
        return 2
    print(f"[{group}] device={dev} pdft={pdft.__version__}", file=sys.stderr, flush=True)
    ...
    out: dict = {"group": group, "device": str(dev),
                 "gpu_name": os.popen(
...
# ===========================================================================
def orchestrate() -> int:
    import numpy as np  # noqa: F401  (kept jax-free in the parent)

    results: dict = {}
    for g in GROUPS:
        print(f"[parent] running group '{g}' (isolated process) ...", flush=True)
        proc = subprocess.run([sys.executable, str(HERE / "profile_script.py"),
                               "--group", g], env=os.environ,
    ...
    if not (light and vag and step):
        print("[parent] FATAL: a group failed; cannot aggregate. See tails above.",
              flush=True)
        # Still dump partials for debugging.
    ...
    summary = {
        "basis": "dct4_canonical", "dataset": "div2k_8q", "m": M, "n": N,
        "dtype": "complex128", "device": step["device"], "gpu_name": step["gpu_name"],
        "note_card": "profiled on this card; the baseline 1008-step run is on a "
                     "separate card (see scratchpad timing). FP64 penalty class is "
                     "the same across A6000/RTX-6000-Ada.",
...
def _write_report(s: dict) -> None:
    ...
    lines = [
        ...
        "",
        f"Canonical `pdft.DCT4Basis({s['m']},{s['n']})` (PR #22), batch {s['batch_size']}, "
        ...
        f"only {p['optimizer']:.1f}% of each step; forward+backward through the circuit is "
        f"{non_opt:.1f}%. The optimizer cost is also batch-independent (it touches the tiny "
        f"2x2/4x4 gate tensors, not the {s['batch_size']} images), so at the real batch its "
        "share is small.",
        "",
        "### Inside the forward",
        "",
        ...
        "(cross-check vs the real 1008-step run on the other card).",
        "",
        "## Why it is slow",
        "",
        f"1. **FP64 (`{s['dtype']}`).** The whole pipeline is complex128. On RTX 6000 Ada / "
        "A6000 (consumer / pro-viz silicon) FP64 runs at ~1/32-1/64 of FP32, so every "
        "contraction and the gradient run far below peak. Phase-independent tax; the single "
        "biggest lever.",
        f"2. **Two full circuit contractions per image** (forward + inverse) over a "
        f"2^16=65536 state through {s['n_gates']} gates, vmapped over {s['batch_size']} "
        "images, then differentiated.",
        ...
        "XLA_PYTHON_CLIENT_PREALLOCATE=false \\",
        "PYTHONPATH=/workspaces/parametric-dft-paper/pdft-dct4main/src \\",
        "  /workspaces/pdft-benchmarks/.venv/bin/python "
        "results/profiling/dct4_div2k_8q/profile_script.py",
        "```",
        "",
        f"Raw numbers: `profile.json`. Device trace (TensorBoard/Perfetto): `trace/` "
